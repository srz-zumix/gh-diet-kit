...
When the restore finishes it writes a `metadata.restored.json` file into `--input-dir` listing only the assets that still need work (their old URL is still present at the destination and they were not uploaded this run); already-restored assets are omitted, and each remaining asset's location ID is rewritten to the destination comment ID resolved during the run. Re-run the restore with `--continue` to resume from `<input-dir>/metadata.restored.json` without re-searching already-migrated comments. The file is not written in `--dryrun` mode.
